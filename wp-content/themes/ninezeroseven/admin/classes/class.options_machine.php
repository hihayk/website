		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'side-bars':
					...
					$output .= '<ul class="nzs_side_bars">';

					foreach ($value['options'] as $key => $name) {

						$output .= '<li><strong>'.$name.'</strong>';

						$output .= '<input type="hidden" id="'.$value['id'].'['.$key.']" name="'.$value['id'].'['.$key.']" value="'.$name.'"/>';

						
						if( $key != 'sidebar-1' ){
							$output .= '<a href="#" class="remove-sidebar">Delete</a>';
						}
					...
					// print_r($smof_data[$value['id']]);
					$output .= '</ul>';
					$output .='<a title="Add Sidebar" class="button" id="nzs_add_sidebar" href="#">Add Sidebar</a>';
				break;
				...
				case 'typography':
					...
					if(isset($typography_stored['face'])) {
						...
						$output .= '<select class="select of-input google_font_select" name="'.$value['id'].'[face]" id="'. $value['id'] .'_face">';
						...
						$g_text = 'The brown fox jumped over the bridge.';
					}
					...
					$output .= '<p class="'.$value['id'].'_face_ggf_previewer google_font_preview" '. $g_size .'>'. $g_text .'</p>';
					...
					if(isset($typography_stored['color'])) {
						...
						$output .= '<div id="' . $value['id'] . '_color_picker" class="typography-color"><div style="background-color: '.$typography_stored['color'].'"></div></div>';
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ','smof_framework').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
