...
jQuery(document).ready(function($){
	...
	jQuery('.nzs_side_bars a.remove-sidebar').live("click",function(){
		jQuery(this).parent('li').remove();
	});

	jQuery('#section-nzs_side_bars a#nzs_add_sidebar').click(function(){
		var newSideBar = prompt('Enter Name For SideBar');

		var newSideBarTitle = newSideBar.replace(/[^a-zA-Z0-9\- ]+/g,"");

		newSideBar = newSideBar.toLowerCase();
		newSideBar = newSideBar.replace(/ /g,"-");
		newSideBar = newSideBar.replace(/[^a-zA-Z0-9\-]+/g,"");

		jQuery('.nzs_side_bars li input').each(function(){

			if(jQuery(this).attr('id') == 'nzs_side_bars['+newSideBar+']'){
				newSideBar = null;
				alert('ERROR! Please use a unique sidebar name');
		...
		if(newSideBar != null && newSideBar !=""){
			

			jQuery('.nzs_side_bars').append('<li><strong>'+newSideBarTitle+'</strong><input type="hidden" value="'+newSideBarTitle+'" id="nzs_side_bars['+newSideBar+']" name="nzs_side_bars['+newSideBar+']"><a class="remove-sidebar" href="#">Delete</a></li>');
