...
class authorsure {
	...
	private static function about_author($id) {
		...
		return sprintf( '<h4>%1$s%2$s</h4>', $about, self::get_author_link($id));
	}
	...
	private static function get_footnote($id, $last_updated_time, $last_updated_date) {	
		...
		return sprintf( '<p id="authorsure-last-updated" class="updated" itemscope="itemscope" itemtype="http://schema.org/WebPage" itemid="%1$s">%2$s %3$s%4$s.</p>', 
			get_permalink(), authorsure_options::get_option('footnote_last_updated_by'), $author, $updated_at);
	}
